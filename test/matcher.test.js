...
const TRIGGERS = [
  { kind: TRIGGER_KIND.NAME, label: 'Afreedi', alias: 'Afreedi' },
...
test('normalize lowercases, strips punctuation, collapses whitespace', () => {
  assert.equal(normalize('  Hey,  Afreedi!!  '), 'hey afreedi');
});

test('normalize folds possessives so "Afreedi\'s" still matches', () => {
  assert.equal(normalize("Afreedi's"), 'afreedis');
  assert.equal(normalize('Afreedi’s'), 'afreedis');
});
...
test('doubleMetaphone returns two codes and is stable', () => {
  const [p, a] = doubleMetaphone('Afreedi');
  assert.ok(p.length > 0);
  assert.deepEqual(doubleMetaphone('Afreedi'), [p, a]);
});

test('doubleMetaphone ignores case and non-letters', () => {
  assert.deepEqual(doubleMetaphone('afreedi'), doubleMetaphone("A-Freedi!"));
});

test('phoneticallyEqual rejects empty input rather than matching everything', () => {
  assert.equal(phoneticallyEqual('', 'afreedi'), false);
  assert.equal(phoneticallyEqual('123', 'afreedi'), false);
});
...
const POSITIVES = [
  ...
  ['so a freddy can you walk us through the gateway', 'Afreedi'],
  ['afridi what do you think', 'Afreedi'],
  ['hey afreedy quick question', 'Afreedi'],
  ['i think a freedy owns that piece', 'Afreedi'],
...
test('a genuinely different name is rejected, and the alias field is the fix', () => {
  // "Ahmedi" is a real, distinct surname — phonetically AMT vs AFRT. Matching it
  ...
  assert.deepEqual(strict.scan('i think ahmedi owns that piece'), []);

  const withAlias = new Matcher(
    [...TRIGGERS, { kind: TRIGGER_KIND.NAME, label: 'Afreedi', alias: 'Ahmedi' }],
    { sensitivity: 0.82 },
  );
  assert.deepEqual(
    withAlias.scan('i think ahmedi owns that piece').map((h) => h.label),
    ['Afreedi'],
  );
});
...
test('cooldown suppresses a repeat of the same alias', () => {
  ...
  const t0 = 1_000_000;
  assert.equal(m.scan('afreedi are you there', { now: t0 }).length, 1);
  assert.equal(m.scan('afreedi again', { now: t0 + 5000 }).length, 0);
});
...
test('cooldown expires and lets the alias fire again', () => {
  ...
  const t0 = 1_000_000;
  assert.equal(m.scan('afreedi are you there', { now: t0 }).length, 1);
  assert.equal(m.scan('afreedi one more time', { now: t0 + 20001 }).length, 1);
});

test('cooldown is per-alias, so a different trigger still fires', () => {
  const m = makeMatcher({ cooldownMs: 20000 });
  const t0 = 1_000_000;
  m.scan('afreedi are you there', { now: t0 });
...
test('reset clears cooldown state for a new meeting', () => {
  ...
  m.scan('afreedi are you there', { now: t0 });
  m.reset();
  assert.equal(m.scan('afreedi are you there', { now: t0 + 100 }).length, 1);
});

test('one utterance mentioning an alias twice fires once', () => {
  const m = makeMatcher();
  const hits = m.scan('afreedi i mean afreedi can you confirm');
  assert.equal(hits.filter((h) => h.label === 'Afreedi').length, 1);
});
...
test('flags a name mention with a question mark as directed', () => {
  const m = makeMatcher();
  const [hit] = m.scan('Afreedi, can you walk us through the gateway?');
  assert.equal(hit.directedQuestion, true);
});

test('flags a name mention with an interrogative but no "?" as directed', () => {
  const m = makeMatcher();
  const [hit] = m.scan('so a freddy what is the timeline on that');
  assert.equal(hit.directedQuestion, true);
});

test('flags a solicitation without an interrogative as directed', () => {
  const m = makeMatcher();
  const [hit] = m.scan('afreedi walk us through the cutover please');
  assert.equal(hit.directedQuestion, true);
});

test('does not flag an incidental name mention as directed', () => {
  const m = makeMatcher();
  const [hit] = m.scan('afreedi already shipped that last sprint');
...
test('isQuestionNear ignores cues beyond the proximity window', () => {
  const tokens = ['what', ...Array(30).fill('filler'), 'afreedi'];
...
test('returns strongest match first', () => {
  const m = makeMatcher();
  const hits = m.scan('afreedi can you cover heelius');
...
test('trigger carries the fields the panel and prompt need', () => {
  const m = makeMatcher();
  const [hit] = m.scan('afreedi are you there');
  assert.equal(hit.kind, TRIGGER_KIND.NAME);
  assert.equal(hit.label, 'Afreedi');
...
test('a matcher with no triggers never fires', () => {
  const m = new Matcher([]);
  assert.deepEqual(m.scan('afreedi helios sarah chen'), []);
});

test('two-letter aliases only match exactly, never fuzzily', () => {
  const m = new Matcher([{ kind: TRIGGER_KIND.NAME, label: 'AF', alias: 'AF' }]);
  assert.equal(m.scan('af can you confirm').length, 1);
  m.reset();
...
test('scanning a long buffer stays bounded and still catches a tail mention', () => {
  ...
  const hits = m.scan(`${filler} afreedi can you confirm`);
  assert.deepEqual(hits.map((h) => h.label), ['Afreedi']);
});
